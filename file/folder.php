...
class WhitelistFolderHandler
{
    ...
    function WhitelistFolderHandler($path, $create = false, $mode = false)
    {
    ...
    {
        if (WhitelistFolderHandler::isWindowsPath($path)) {
            return '\\';
        }
        return '/';
    }
    ...
    {
        if (WhitelistFolderHandler::isSlashTerm($path)) {
            return $path;
        }
        return $path . WhitelistFolderHandler::correctSlashFor($path);
    }
    /**
    ...
    /**
     * Returns true if the File is in a given WhitelistPath.
     *
     * @return bool
     * @access public
     */
    function inWhitelistPath($path = '')
    {
        $dir = substr($this->slashTerm(dirname(__FILE__)), 0, - 1);
        $newdir = $dir . $path;
