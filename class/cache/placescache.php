 ...
 * @version         $Id: Placescache.php 12537 2014-05-19 14:19:33Z beckmi $
 */
...
class PlacesCache
{
    ...
    function __destruct()
    {
    	if (!$caches = self::read('caches-systemosity'))
    		$caches = array();
    	...
    	$this->write('caches-systemosity', $caches, 3600 * 24 * 7 * 4 * 12);
    }
    /**
    ...
    {
        if (!class_exists('PlacesCache' . ucfirst($name))) {
    ...
    function engine($name = 'file', $settings = array())
    {
        ...
        $cacheClass = 'PlacesCache' . ucfirst($name);
        $_this =& PlacesCache::getInstance();
