 ...
 *
 * @copyright       (c) 2000-2016 Places Project (www.Places.org)
...
class PlacesLocal
{
    ...
    /**
     * PlacesLocalAbstract::utf8_encode()
     *
     * @param  mixed $text
     * @return string
     */
    ...
    /**
     * PlacesLocalAbstract::convert_encoding()
     *
     * @param  mixed  $text
     * @param  string $to
     * @param  string $from
    ...
    {
        global $PlacesConfig, $PlacesUser;
        ...
        if ($format === 'rss' || $format === 'r') {
            $TIME_ZONE = '';
            if (isset($GLOBALS['PlacesConfig']['server_TZ'])) {
                $server_TZ = abs((int)($GLOBALS['PlacesConfig']['server_TZ'] * 3600.0));
                $prefix    = ($GLOBALS['PlacesConfig']['server_TZ'] < 0) ? ' -' : ' +';
        ...
        if ($timeoffset === null) {
            $timeoffset = ($PlacesConfig['default_TZ'] == '') ? '0.0' : $PlacesConfig['default_TZ'];
        }
    ...
    /**
     * PlacesLocalAbstract::number_format()
     *
     * @param  mixed $number
     * @return mixed
     */
    ...
    /**
     * PlacesLocalAbstract::money_format()
     *
     * @param  mixed $format
     * @param  mixed $number
     * @return mixed
...
class PlacesLocalWrapper
{
    ...
    {
        if (class_exists('PlacesLocal')) {
            return true;
        }
        include_once __DIR__ . DIRECTORY_SEOERATOR . 'placeslocal.php';
