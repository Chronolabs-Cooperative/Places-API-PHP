...
This goes in your API_ROOT_PATH/.htaccess file
    ...
    RewriteEngine On
    RewriteRule ^v([0-9]{1,2})/([a-zA-Z]+)/([a-zA-Z]+)/([0-9]+)/(raw|html|serial|json|xml).api ./index.php?version=$1&country=$2&place=$3&radius=$4&output=$5 [L,NC,QSA]
    RewriteRule ^v([0-9]{1,2})/([a-zA-Z]+)/([a-zA-Z]+)/(raw|html|serial|json|xml).api ./index.php?version=$1&country=$2&place=$3&output=$4 [L,NC,QSA]
